...
import jp.ac.nagoyau.is.ss.kishii.ob.team.agent.fb.SecondaryFireBrigade;
import jp.ac.nagoyau.is.ss.kishii.ob.team.center.fs.RandomCommandFireStation;
 ...
 */
public final class LaunchOBRandomCenterAgents {
	...
	private LaunchOBRandomCenterAgents() {
	}
	...
	private static void connect(ComponentLauncher launcher, int fb, int pf,
		...
		try {
			while (fb-- != 0) {
				...
				launcher.connect(new SecondaryFireBrigade());
		...
		try {
			while (at-- != 0) {
				...
				launcher.connect(new RandomCommandFireStation());
				System.out.println("success");
			}
		} catch (ComponentConnectionException e) {
			System.out.println("failed: " + e.getMessage());
